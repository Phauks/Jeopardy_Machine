...
<<<<<<< HEAD
- [x] **The real front door** (2026-08-15; REBUILT 2026-08-16, see below): `/` became a designed landing with the room-code box as the primary control and the dev-surface index demoted into a closed drawer, and `/lobby` became the room browser - title AND host label as different facts, capacity meters, lock, phase badge, age, and the password prompt inline in the card it belongs to, with empty / not-yet-loaded / registry-broken as three different screens (docs/decisions/2026-08-14-room-visibility-and-lobby.md, amended). The two-page split is what the 2026-08-16 rebuild reversed; everything else on this line survived into it
...
- [x] ~~**Character select and team joining as their own screens**~~ - **superseded 2026-08-16, one day later, by the persistent-layout law** (see below). The identity moment did need room; splitting the screen was the wrong way to give it.
>>>>>>> worktree-agent-a49a5f05f1996fb61
...
<<<<<<< HEAD
      Progress 2026-08-16 - the pre-game rework (docs/decisions/2026-08-16-persistent-layout-and-pregame-rework.md), batch item 1:

- [x] **The standing layout law adopted**: state changes IN PLACE, surfaces do not swap, and space is reserved so an arriving answer changes words rather than positions. Held on the front door by a source gate (reserved blocks + hard measures on prose)
- [x] **One front door**: `/lobby` folded back into `/` and was DELETED with no redirect (no-legacy directive). Rejoin offer, code box + password, create form and the live public list are one screen - two columns on a laptop, priority-stacked on a phone - and a complete code still WINS over the list beside it
- [x] **Create a room from the front page**: a real form (name with a live counter, host label, public/private, optional password, player cap, spectators) posting to `POST /api/rooms` with the sample game dynamically imported at the tap, then handing off to `/room/<CODE>/host` with the host token stashed per-code in sessionStorage. The one case that does NOT hand off: a public room the registry could not list says so, with its code and the fix
- [x] **Rejoin memory**: this tab remembers the rooms it entered (`room-memory.ts`, sessionStorage, no token and no account) and the front door offers them back by name, after asking the new `GET /api/rooms/<CODE>/live` whether each still exists. Dead entries clean themselves up silently; a registry that cannot answer means UNKNOWN and keeps the offer, never deletes it
- [x] **Art direction on the token system**: the page is built from the board's own materials (category-blue masthead, ink panels on thick gutters, listed rooms drawn as cells, numbers in the value face and value color, square corners) with one declared type scale and spacing rhythm - which also makes it correct under all four presets rather than only the dark ones. Hero copy restructured to a short balanced lead plus a supporting line so it stops reflowing raggedly; the password helper prose is now just "Password"
- [x] **A shared way back**: `src/lib/chrome/home-button.svelte` (anchor, never `history.back()`; inline or floating; optional confirm), adopted by the `/dev` layout - the play surfaces adopt it in their own passes
=======
      Progress 2026-08-16 - the persistent-layout law and the pre-game rework (docs/decisions/2026-08-16-persistent-layout-and-pregame-rework.md):

- [x] **The pre-game is ONE screen**, per the standing UI law the owner set: state changes in place, surfaces do not swap. Character, teams and the room roster are three regions of `pre-game-screen.svelte`, all mounted from the first paint - choosing a colour no longer navigates away from the teams and joining a team no longer hides your character. Taking a seat changes what the identity controls MEAN (draft -> live), not what is on screen, which retires the post-join identity-sheet modal too. `playerRouteStageFor` and its four screens are deleted; `playerSurfaceFor` answers pre-game or buzzer, and the buzzer is the exception the law names (the game changing state, not navigation). Every region reserves its space, gate-tested
- [x] **The accent tints the character, not the backdrop** - the owner's bug, root-caused. The 216 per-accent stills were always right; the animated preview rendered the accent-NEUTRAL walk sheet and put the accent on its round backing, which the component's own header called an accepted trade. It is the biggest thing on the screen. The fix costs no committed bytes: the per-accent still carries the first paint, and the walk sheet is recoloured in the browser through the bake's own `palette-recolor` over the bake's own targets, so a recoloured frame and a baked still agree by construction. The test that should have caught it asserted the opposite behaviour, and now asserts the painted image changes with the accent
- [x] **Skin tones for the human models**: a curated six-step ramp on the same runtime recolor, humans only, an explicit choice with a neutral (pack-colours) default and no inference anywhere. Not baked - a tone axis on top of the accent axis would be ~6.5 MB of stills against a 2 MB budget; the arithmetic is in the bake README. A manifest gate holds the skin cells and every human's accent target apart by more than either tolerance, so the two controls cannot repaint each other
- [x] **Real team management for players**, all in place on the same screen: create (whether or not you already have a team), **move** to another team after joining (one message, so the room never sees you briefly teamless), leader **rename** inline on the card, and leave to the holding area - rename, lock and leave behind the team's "..." per the overflow rule. Additive protocol only: `skinToneId`, `team-leave`, and a `teams-full` refusal reason that the store enforces as well as the form, because the last slot can go between the render and the tap
- [x] **Laptops and desktops as first-class**: container queries (the surface's own width, not the device's) give one column on a phone, two at 48rem, and three genuine side-by-side regions at 64rem, with the action bar unsticking and the character preview growing. Gate-tested, because a wide layout is one line from being deleted as unused and no markup test would notice
>>>>>>> worktree-agent-a49a5f05f1996fb61
