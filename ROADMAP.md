...
- [x] **Teams are askable**: whether a room plays in teams is matrix row 34 - a RULE - so it was fixed by the game definition and every room the front door made was an individuals room, with the player screen's teams region able only to say so (owner report 2026-08-19). The create form now asks, and writes the answer as a rules override onto the game rather than as a second room-level flag
