...
>
> Last updated: 2026-08-17 (THE RECONCILE: the play surfaces now talk to real rooms - one code, one GameRoomDO, many devices; before that, staged-lobby placement fixes + the host settings cog + the C4 verification walk; before that, the reconcile between the two parallel milestones: the spectator budget reaches the lobby row, the theme document gains its `environment` + `staging` slots, streamer mode reaches the display, and the room's caps refuse in human copy. M1 editor phase open; M0 awaits owner's first manual deploy)
