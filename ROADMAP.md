...
- [x] **The front door, rebuilt as one counter** (docs/decisions/2026-08-18-front-door-architecture.md, after the owner rejected two versions; research: docs/research/06-join-flow-patterns.md): `/` stops being four panels of equal weight under a hero. ONE field joins by code and searches the list, the list hangs beneath it as that field's results, the password is a state of having a code rather than a box beside it, hosting is a button whose form opens in place, and the masthead is a 48px wordmark strip with the developer index behind its gear. What a typed string means is `src/lib/lobby/room-filter.ts`, pure and tested. The hero, the three pillars, the numbered panel headings, the bottom dev drawer, the page footer's pitch line and three-fact strip, `rejoin-panel.svelte`, `room-code-field.svelte`, `dev-menu.svelte` and `room-search.ts` are deleted - and the owner's earlier copy deletions, the two required name fields, the printed cap bounds and the clock-time freshness stamp survived the rebuild, held by their own gates
...
- [x] **The password door** (deferred at the reconcile, closed 2026-08-19): a phone reaching a PASSWORD room by URL alone carries nothing - the front door stashes a password beside the code, and a QR scan or a pasted link has never been through the front door. It was told "this room needs a password, the host has it" with nowhere to type one, which is a dead end dressed as an explanation. The protocol always allowed for the fix - both password refusals KEEP the socket precisely so a phone can prompt and retry on it - so `submitRoomPassword` re-knocks on the connection it already has. The first ask and a retry read differently, or a second wrong try looks like a screen that ignored you. What stays true: an unjoined connection is told nothing about the room behind the door, which is what makes a password room a password room
...
- [x] **A game definition can be joined to its pack** (`embedContentPack`): the M1 promise that "importing an external game without its pack is a hard, friendly error" finally has an implementation. The event game is external-pack and the room resolves clue text from an EMBEDDED pack only, so nothing could host the club night until this existed. Two checks that answer different questions - the pack file's sha256 (documents carry no id, so only the bytes can say it is the right pack) and whether the pack actually covers every referenced cell, which a matching hash does not guarantee and whose failure mode is otherwise a cell opening blank mid-game
- [x] **Three seating modes, and the create form asks** (owner, 2026-08-19): matrix row 34 gains `mixed` beside `individuals` and `teams` - teams exist AND playing solo is a legitimate final answer, so a room holds three couples and two people who came alone without a team being invented for either. It was a boolean everywhere (`teamsMode`) because two modes hid the fact that every call site was asking one of two DIFFERENT questions; they are now `teamsAreOffered` (draw the team machinery) and `teamsAreRequired` (is a teamless player unfinished, or a soloist), and mixed is the only mode where those disagree. The engine needed no new concept - a scoring entity has always been `teamId ?? playerId` - only permission to let a teamless player through. The wire carries the mode and refuses a snapshot that omits it. Before this the form did not ask at all, so every room the front door made was an individuals room and the player screen's teams region could only say so
- [x] **Clue media renders** (owner, 2026-08-19: "pictures, videos, audio files, and other files must be renderable"): the wire carried a bare `mediaId`, which is right inside a document and wrong on a wire - a phone holds no document, so it was told there WAS a picture and given nothing to do about it, and every picture clue came out as words. The room resolves the id against the pack it holds and sends kind, type, alt text and a url; `clue-media.svelte` is the one place that becomes pixels, on the display (media first, autoplaying, because the display owns room audio) and the host card (small, silent, plus the answer's own media host-only). `kind` gained **video** and **file** - the note saying video was "deliberately absent until a mode needs it" is reversed. No bytes is a STATE: the alt text shows, never a broken frame
- [x] **A host chooses which game the room opens with** - the built-in sample, the club night, or a `.game.json` they were sent (`game-catalog.ts`). Every path ends in a SELF-CONTAINED definition, because a room resolves clue text and media from an embedded pack only: an external game is joined to its pack in the browser, and `bundled` media paths - which are statements about the authoring device's disk - become URLs thirty phones can fetch. Which file is which is decided by the FORMAT inside, never the filename, so picking the game and the pack in either order works. A brought game's media reaches the room without a url and shows its alt text rather than a broken frame; uploading those bytes is R2's job (below)
- [x] **The event's shippable documents moved to where they are served** (`apps/web/static/games/board-game-club-x-els/`) rather than being copied there - one canonical location, `bundled` paths still relative to the pack beside them, and the referential-integrity gate follows them. `events/` keeps the README and the curation record
- [ ] Still owed for the exit criteria: the owner's three look-at-the-picture checks and the lobby track pick (below), then a dress rehearsal on real phones
