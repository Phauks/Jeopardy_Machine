...
| `packages/bots/`           | Bot players (M3): headless clients speaking the real room WS protocol with seeded behavior; drives the realtime/e2e suites and a CLI (`pnpm -F @jeopardy/bots bots`). The M4 sim panel builds on it.                                                                         |
| `tools/avatar-bake/`       | Offline avatar pipeline (Kenney CC0 packs -> committed webp sprites, walk-cycle sprite sheets, GLB models + manifest). Tooling only, no build/test scripts, never bundled.                                                                                                   |
| `tools/audio-bake/`        | Offline sound pipeline (Freesound CC0 sources -> license re-verified, trimmed, normalized, uniform-onset MP3s under `apps/web/static/sounds/` + manifest + credits). Same shape and same rules as avatar-bake.                                                               |
| `tools/event-media-bake/`  | Offline image pipeline for the event picture round (Commons originals -> license + sha1 re-verified, downscaled WebP under `apps/web/static/games/board-game-club-x-els/media/`, pack media refs and the game's pack hash rewritten).                                        |
