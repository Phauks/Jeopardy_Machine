...
**The accent tints the CHARACTER, never the backdrop behind it** (owner, 2026-08-16). The stills have always been baked per accent and were always right; the animated walk-cycle preview was not, because there is one sheet per avatar in pack colors, and the accent was put on its round backing instead. The fix keeps the byte budget and recolors the sheet in the browser through the same `palette-recolor` the bake uses (`sheet-recolor.ts`), so a recoloured frame and a baked still agree by construction. The backing is a neutral surface with a thin accent ring - enough to read as the same object as the roster chip, not the place the colour lives.

**Skin tone** is a second, independent axis on the human models only (`tools/avatar-bake/src/skin-tone-palette.mjs`): a curated six-step ramp, an explicit choice, defaulting to the pack's own colors and never inferred from anything. It rides the same runtime recolor and is deliberately not baked - the byte arithmetic is in that tool's README. Accent and tone target disjoint colormap cells, held apart by a manifest gate with more margin than either tolerance, so the two controls can never repaint each other's pixels. Player accents are a player's own choice and deliberately do NOT re-derive from the active theme - but the palette must contain every preset's `accentColor` (bake-time check + `avatar-manifest.gate.test.ts`), so accent-matching-theme always has an exact sprite. Adding a preset with a new `accentColor` therefore includes a palette addition + re-bake (`tools/avatar-bake/README.md`).
