...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `/`                    | **The front door (A1), and the only one** (2026-08-16): rejoin offer, code box + password, create-room form, and the live public-room list, all on one screen. Polls `/api/rooms`; probes `/api/rooms/<CODE>/live` for each remembered room. `/lobby` folded back in here and was DELETED - no redirect (no-legacy directive)                 | none       |
...
| `/room/[code]/host`    | C4 console: minimap, ARM (spacebar), judge row (arrows), answer visible, undo, score drawer, pause, DD + Final wizards, manual mode, **the game-screen setup + join panel** (C1/C2, 2026-08-19), **mirror mode** (C1b, `?mirror`), dev sim panel                                                                                              | `host`     |
...
| Password                                      | room   | Changing it never disconnects anyone already in - which is exactly why it is a mid-game control                       |
...
- `host-settings/host-settings.test.ts`: the two type scales moving independently, the tolerant parse and the one-key storage document, that a device preference never changes the room and a room setting never lives on the device, both room-settings refusals, and the panel labelling its two halves.
...
- The console's room-settings half writes through `RoomStore.updateRoomSettings`, which the ws store still stubs - both server doors (the host-only message and PATCH /api/rooms/<CODE>) exist and are tested; the wiring is reconcile work like every other store method.
