...
> Stamp: verified against the pre-game journey work (landing, lobby browser, character/team screens, staged lobby, mobile display), 2026-08-15 UTC. Prefer the reproduce-commands over the copied numbers - if they disagree, this file is stale; fix it in the same commit as whatever changed.
...
| All tests green: 5 packages, 758 tests (194 protocol, 170 engine, 6 bots, 321 web, 67 realtime-in-workerd)                               | `pnpm test`                                                                                                                                                       |
...
| The public lobby lists live public rooms from D1 (registry rows are a cache; the DO refuses dead rooms regardless)                       | `pnpm -F @jeopardy/realtime exec vitest run test/registry.test.ts` (real D1 + the web app's migration); browser: `/lobby` and the harness's "List public rooms"   |
...
- No R2 media flows exist yet; the R2 binding sits unused by code. D1 is in use for the room registry ONLY, and only once the owner applies `apps/web/migrations` by hand (docs/cloudflare-setup.md §2a) - until then the lobby answers empty (now with `registry.reason = "no-table"` saying so) and rooms work by code as before. **The migration has not been applied to any deployed environment yet** - that is the known cause of an empty lobby on the live site.
...
| Claim                                                                                                                                          | Reproduce                                                                                                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `/` is the real landing: code box primary, `/lobby` secondary with a live count, dev index collapsed but complete                              | `pnpm dev`, open http://localhost:5173/ ; `landing-screen.test.ts` holds the priority order and the drawer                         |
| `/lobby` distinguishes rooms-listed, nobody-hosting, not-fetched-yet, and registry-broken as four different screens                            | `pnpm -F @jeopardy/web test` (`lobby-screen.states.test.ts`); live with a registry: `pnpm dev:rooms`, http://localhost:8788/lobby  |
| A room card types the title and the host label as different facts, meters capacity, and prompts for a password inside its own card             | `lobby-screen.states.test.ts`; spectator seats appear only when the wire carries them (`room-capacity.ts` reads them structurally) |
...
Not yet true: which staging theme a room uses is a local id (`?staging=campfires` on the player and display routes) rather than a theme-document field - the one-line `themeBodySchema` addition is written out in `staging-theme-registry.ts`, beside the `environment` reservation. The staged lobby's 3D half has no automated browser test for the same reason the diorama does not (CI has no chromium); `/dev/diorama` is the standing check.
