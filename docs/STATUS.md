...
> Stamp: verified against the lobby-diagnostics + room instrument panel work, 2026-08-14 UTC. Prefer the reproduce-commands over the copied numbers - if they disagree, this file is stale; fix it in the same commit as whatever changed.
...
| All tests green: 5 packages, 612 tests (194 protocol, 170 engine, 6 bots, 175 web, 67 realtime-in-workerd)                               | `pnpm test`                                                                                                                                                                    |
...
- No play SURFACES exist: rooms run and phones could connect, but the buzzer/board/console UIs are M4 - the only clients today are the /dev/rooms instrument panel, bots, and test scripts.
