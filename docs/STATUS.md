...
> Stamp: verified against the M3 realtime-rooms milestone, 2026-08-14 UTC. Prefer the reproduce-commands over the copied numbers - if they disagree, this file is stale; fix it in the same commit as whatever changed.
...
| All tests green: 5 packages, 354 tests (118 protocol, 170 engine, 6 bots, 27 web, 33 realtime-in-workerd)   | `pnpm test`                                                                                                                                                       |
...
- Engine timer bars render from `timer-set` events; expiries are server-driven - clients never dispatch timeout actions.
...
- No R2 media flows exist yet; the D1/R2 bindings sit unused by code (web wrangler.jsonc carries the owner-provisioned ids).
