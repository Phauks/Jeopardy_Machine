...
**Reconciled 2026-08-16: the environment slot is a real theme-document field.** `themeBodySchema` grew `environment: z.enum(["none", "studio", "forest", "pirate", "dungeon"]).optional()` - the line `diorama-environment.ts` had written out - together with the staged lobby's `staging` slot, in one protocol change and with no migration (both optional, `soundSet`'s precedent). The display route now reads `theme.environment` and passes it through `resolveDioramaEnvironment`, which is the only piece that stays local and the piece that matters: a theme document is DATA, so a theme may legitimately name scenery whose kit has not shipped, and every such value resolves to the studio stage rather than to a blank projector. Only `none` is honoured exactly, because only `none` means "no diorama". `?environment=` and `?staging=` survive as dev overrides that WIN over the document, so a preset can still be reviewed against any stage without editing a document. Terra Verde is the first preset to use the slots for real (`environment: "forest"`, `staging: "campfires"`) and renders on the studio stage until the kit lands - no screen edit on that day.
