...
**Amended 2026-08-15, and REVERSED 2026-08-16 - the list is back on the root, and `/lobby` is gone.** The 2026-08-15 amendment split the two: `/` became the landing with a code box and a link carrying a live count, and `/lobby` became the browser with the code box repeated at its top. The owner's verdict after using it: **browsing and joining are the same act**, and a link with a number on it is a worse answer to "is anyone playing?" than the rooms themselves. So the split is undone (docs/decisions/2026-08-16-persistent-layout-and-pregame-rework.md, "Landing and lobby"):

- **`/`** is the whole front door: the rejoin offer, the code box + password field, the create-room form, and the live public list, all present at once. The code box still WINS - a complete code holds the list back rather than competing with it - which was the only part of the 2026-08-15 split worth keeping, and it did not need a second page to be true.
- **`/lobby` was deleted outright**, with no redirect kept (docs/research/00-user-directives.md, "No legacy code"). Its screen component went with it; the list itself survives as `src/lib/lobby/room-browser.svelte`, a REGION of the front door, and every convention below still lives there: lock, capacity meter, phase badge, age, dimming, newest-first, and the per-card password prompt.

What the split was right about is preserved rather than lost: the two jobs no longer compete because they are laid out as two COLUMNS (control column, list column) on a laptop and stacked in priority order on a phone - a layout answer to a layout problem, instead of a navigation answer to one.
...
## Addendum 2026-08-16: the registry answers one more question - "is that room still there?"

The front-door rebuild needs a fact the listing cannot supply: this browser remembers a room it was in, and it must not offer to walk someone back into a game that ended during the coffee break. The listing answers that for PUBLIC rooms only, and the default is private.

`GET /api/rooms/<CODE>/live` -> `{code, live, registry}` (`apps/web/src/routes/api/rooms/[code]/live/+server.ts`). It reads exactly the liveness half of the listing predicate against one row (`registryRowIsLive()`: not ended, phase in lobby/active, not past its expiry) and deliberately says NOTHING else - no title, no phase, no counts, no listing. Three properties keep it inside this document's posture:

- **No host token**, unlike the sibling diagnostics `GET /api/rooms/<CODE>`. It answers a question anyone holding the code can already answer by opening the socket and being refused with `no-such-room`, so it adds no oracle - and it costs one D1 read instead of a WebSocket handshake. It is a code oracle only in the sense the room protocol already was.
- **A cache read, and known to be one.** Drift is survivable in both directions: a row that outlived its room costs one wasted tap and a refusal from the DO, and a room whose row never landed reports `registry: unavailable`, which the client renders as UNKNOWN and never as dead (`src/lib/lobby/room-liveness.ts`: `live | gone | unknown`). Treating "the registry cannot answer" as "the room is dead" would silently delete a rejoin offer for a room sitting right there - the same class of bug as addendum (b), one layer up.
- **Never cached**, because a rejoin offer for a room that ended thirty seconds ago is precisely what it exists to prevent.
