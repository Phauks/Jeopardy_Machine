...
**The dev index is a header control.** It moved out of the drawer at the foot of the page into a gear menu in the masthead (a `<details>` so it works with no JavaScript and server-renders closed; it lived in `dev-menu.svelte` until the 2026-08-18 rebuild folded it into `masthead-bar.svelte`, where the wordmark strip it belongs to now lives). The owner rule is unchanged - every meaningful surface gets an entry, in the same PR that ships it, still one line in `surface-cards.ts`. Only the place moved, and the drawer is gone rather than duplicated: the last thing a visitor scrolls past should not be a list of engineering routes.

**The public list is searchable.** Instant and local to the fetched rows (`room-search.ts` until 2026-08-18, when the rebuild merged the search box INTO the single entry field and the rules became `src/lib/lobby/room-filter.ts` - the reasoning below is unchanged and still the reason it is client-side): the listing is already capped at `limits.lobby.listingMax`, so asking a server to re-filter forty rows a keystroke at a time would be slower, would show a stale answer while it flew, and would turn a browse surface into a query endpoint. Terms are AND-ed against one haystack of title + host label, because someone searching "board game club" does not know which of the two strings carries the words. A query that matches nothing says so and offers the whole list back, rather than borrowing the "nobody is hosting" state.
