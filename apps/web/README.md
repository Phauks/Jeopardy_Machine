...
- `/dev/echo` is the dev-loop smoke page (docs/DEVELOPMENT.md); `/dev/theme` is the theme gallery (live preset switcher over the token contract - docs/design/theming.md). `/dev/*` routes never get linked from product UI.
