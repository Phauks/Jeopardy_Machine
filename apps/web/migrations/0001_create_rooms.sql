...
--
-- Deliberately absent: anything identifying a player. Room passwords were removed entirely on
-- 2026-08-20 (@jeopardy/protocol room/visibility.ts) - the code is what admits people - so the
-- has_password column went with them.
--
...
--
-- REWRITTEN AGAIN 2026-08-16 (the reconcile): the spectator columns below, and AGAIN on
-- 2026-08-20 to drop has_password. Batched into THIS file rather than a 0002 each time: the
-- table is a cache of rooms that live hours, so a DROP + CREATE loses nothing anybody wants,
-- and one re-apply covers every edit - which is one interruption of the owner instead of three.
--
