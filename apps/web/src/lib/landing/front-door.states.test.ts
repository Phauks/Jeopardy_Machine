...
type Overrides = {
  ...
  /** The clock the "updated Xm ago" line is measured against. */
  now?: number;
};
...
describe("one counter, one list, one host button", () => {
  ...
  it("orders the page: counter first, list beneath it, the rest after", () => {
    expect(body.indexOf("Room code")).toBeLessThan(body.indexOf("Public rooms"));
    expect(body.indexOf("Public rooms")).toBeLessThan(body.indexOf("No app, no account"));
  });
  ...
  it("keeps the no-accounts promise, as one line rather than a marketing band", () => {
    expect(body).toContain("No app, no account");
    expect(body).toContain("None, ever");
  });
...
describe("the live listing states", () => {
  ...
  it("keeps the real fetch timestamp on screen", () => {
    expect(renderFrontDoor()).toContain("Updated just now");
    expect(renderFrontDoor({ now: fetchedAt + 5 * 60_000 })).toContain("Updated 5m ago");
  });
...
describe("hosting is a button, and its form opens in place", () => {
  ...
  it("explains what being public means before anyone chooses it", () => {
    ...
    expect(working).toContain("Only people you give the code to can join");
    const publicForm = { ...blankCreateForm(), listing: "public" as const, title: "Quiz" };
    expect(
      renderFrontDoor({ createForm: publicForm, createState: { status: "creating" } }),
