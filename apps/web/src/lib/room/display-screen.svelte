<script lang="ts">
  ...
  // A QR is only drawn from an ABSOLUTE url. With no origin this string is the bare path
  // `/room/BQKX7`, which encodes into a QR that scans perfectly and goes nowhere - the exact
  // shape of the owner's 2026-08-20 report, and silent because the picture looks right. The
  // route passes `page.url.origin`, which exists in SSR and in the browser alike; this is the
  // belt (src/lib/room/join-share.ts).
  const scannable = $derived(isScannableJoinUrl(joinUrl));
  const qrSvg = $derived(codeHidden || !scannable ? "" : renderSVG(joinUrl, { border: 2 }));
