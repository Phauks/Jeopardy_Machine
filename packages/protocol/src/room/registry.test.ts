...
import { lobbyListingSchema, roomSummarySchema } from "./registry.ts";
...
describe("lobby listing", () => {
  it("caps a listing at the operational limit (pagination is deliberately deferred)", () => {
    ...
    expect(lobbyListingSchema.safeParse({ rooms, fetchedAt: Date.now() }).success).toBe(true);
    expect(
      lobbyListingSchema.safeParse({ rooms: [...rooms, summary], fetchedAt: Date.now() }).success,
    ).toBe(false);
  });

  it("parses an empty lobby (the common case on a quiet night)", () => {
    expect(lobbyListingSchema.parse({ rooms: [], fetchedAt: 1 })).toEqual({
      rooms: [],
      fetchedAt: 1,
    });
  });
